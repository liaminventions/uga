...
unsigned long channel_5_fs = 1000; //gear, less than 1500 = disarmed
...
float maxYaw = 190.0;     //Max yaw rate in deg/sec
// For land speeder, these ^^^ don't do anything!

float Kp_roll_angle = 0.44;    //Roll P-gain - angle mode 
float Ki_roll_angle = 0.25;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.05;   //Roll D-gain - angle mode (has no effect on controlANGLE2)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.44;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.25;   //Pitch I-gain - angle mode
...
float Kp_yaw = 0.38;          //Yaw P-gain
float Ki_yaw = 0.09;          //Yaw I-gain
float Kd_yaw = 0.00011;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
...
float maxTurnRate_0thro = 160.0; // this is the maximum yaw rate for turns at zero throttle, deg/s
float maxBankAngle_0thro = 18.0; // this is the maximum bank angle into a turn at zero throttle, deg
float maxTurnRate_100thro = 100.0; // this is the maximum yaw rate for turns at full throttle, deg/s
float maxBankAngle_100thro = 25.0; // this is the maximum bank angle into a turn at full throttle, deg
...
#endif
#if defined USE_CRSF_RX
  HardwareSerial &crsfSerial = Serial3; 
  AlfredoCRSF crsf;
...
void setup() {
  ...
  delay(500);
  Serial4.begin(115200); //for tfmini 
  delay(10);
  lidar.begin(&Serial4); //pass lidar serial port to tfmini library object
  delay(10);
...
void loop() {
  ...
  //Throttle cut check
  throttleCut(); //Directly sets motor commands to low based on state motorFade
...
void controlMixer() {
  ...
  if (channel_5_pwm > 1500) { // DISARMED
    motorFade = floatFaderLinear(motorFade, 0.0, 1.0, 2.0, 0, 2000);
  }
  else { //ARMED
    motorFade = floatFaderLinear(motorFade, 0.0, 1.0, 4.0, 1, 2000);
  }
  //Serial.println(motorFade);


  float hover_thro = 0.3; //0.15
   
  //Quad mixing
  // m1_command_scaled = motorFade * (-pitch_PID + roll_PID - yaw_PID + alt_PID + hover_thro); // Front Left
  // m2_command_scaled = motorFade * (pitch_PID + roll_PID + yaw_PID + alt_PID + hover_thro); // Back Left
  // m3_command_scaled = motorFade * (-pitch_PID - roll_PID + yaw_PID + alt_PID + hover_thro); // Front Right
  // m4_command_scaled = motorFade * (pitch_PID - roll_PID - yaw_PID + alt_PID + hover_thro); // Back Right
  // m5_command_scaled = motorFade * (thro_des); // Pusher Prop
  // m6_command_scaled = 0;
  ...
  // Thrusters
  s1_command_scaled = motorFade * (thro_des - yaw_PID); // Left Thrust
  s2_command_scaled = motorFade * (thro_des + yaw_PID); // Right Thrust

  // Airbrakes (WIP)
  s3_command_scaled = 0; // Airbrake Left
  s4_command_scaled = 0; // Airbrake Right
...
void controlAlt() {
  ...
  float alt_setpoint = alt_stpt; //desired altitude, cm 
  float kP_alt = 0.3; //0.7
  float kI_alt = 0.35; //0.35 
  float kD_alt = 0.16; //0.17
  float alt_int_max = 40.0; //integrator saturation limit
  float alt_error_max = 40.0; //cm
  float alt_beta = 0.01; //LP filter parameter

  //get the lidar data
  lidar.getData(tf_alt_raw, tf_trust, tf_temp);

  //transform to absolute altitude given orientation of platform
  float ang_from_level = sqrt(roll_IMU * roll_IMU + pitch_IMU * pitch_IMU);
  
  //filter the altitude data
  tf_alt_filtered = alt_beta*tf_alt_raw * cos(ang_from_level * 0.0174533) + (1.0 - alt_beta)*tf_alt_filtered;
  ...
  if (motorFade <= 0.0) {   //Don't let integrator build if fully disarmed
    alt_integral = 0.0; 
  }
  ...
  alt_PID = constrain(alt_PID, 0.0, 0.4);

  //Serial.print("tf_alt_raw: ");
  //Serial.println(tf_alt_raw); 
  //Serial.print("tf_trust: ");
  //Serial.println(tf_trust);
...
void getDesStateSPEEDER() {
  //DESCRIPTION: Normalizes desired control values to appropriate values, specific to landspeeder control input mixing

  thro_des = (channel_1_pwm - 1500.0)/500.0; //Between 0 and 1
  roll_des = (channel_2_pwm - 1500.0)/500.0; //Between -1 and 1
  pitch_des = (channel_1_pwm - 1500.0)/500.0;; //Between -1 and 1
  yaw_des = (channel_4_pwm - 1500.0)/500.0; //Between -1 and 1
  ...
  //Constrain within normalized bounds and do the control input shaping
  thro_des = constrain(thro_des, 0.0, 1.0); 
  float yaw_des_0thro = -1.0 * constrain(roll_des, -1.0, 1.0) * maxTurnRate_0thro;
  float roll_des_0thro = pow(abs(constrain(roll_des * 1.5, -1.0, 1.0)), 1.5) * sgn(roll_des) * maxBankAngle_0thro + roll_trim;
  float yaw_des_100thro = -1.0 * constrain(roll_des, -1.0, 1.0) * maxTurnRate_100thro;
  float roll_des_100thro = pow(abs(constrain(roll_des * 1.5, -1.0, 1.0)), 1.5) * sgn(roll_des)  * maxBankAngle_100thro + roll_trim; 
  
  pitch_des = constrain(pitch_des, -1.0, 0.0) * maxPitchBraking + pitch_trim;  // same for all throttle

  yaw_des = thro_des * (yaw_des_100thro) + (1.0 - thro_des) * (yaw_des_0thro);
  roll_des = thro_des * (roll_des_100thro) + (1.0 - thro_des) * (roll_des_0thro);
...
void getCommands() {
   ...
   */
    ...
    channel_3_pwm = 1500;//getRadioPWM(3);
    channel_4_pwm = 1500;//getRadioPWM(4);
    channel_5_pwm = getRadioPWM(5);
    channel_6_pwm = 1500;//getRadioPWM(6);
